...
TEST(DILLexerTests, LookAheadTest) {
  StringRef input_expr("(anonymous kw_namespace)::some_var");
  llvm::Expected<DILLexer> maybe_lexer = DILLexer::Create(input_expr);
  ASSERT_THAT_EXPECTED(maybe_lexer, llvm::Succeeded());
  DILLexer lexer(*maybe_lexer);
  Token token = lexer.GetCurrentToken();
  ...
  // sure they are the identifier 'anonymous', the identifier 'kw_namespace'
  // ')' and '::', in that order.
  ...
  EXPECT_EQ(lexer.LookAhead(2).GetSpelling(), "kw_namespace");
  ...
  EXPECT_EQ(token.GetLocation(), strlen("(anonymous kw_namespace)::"));
...
TEST(DILLexerTests, IdentifiersTest) {
  ...
  std::vector<std::string> valid_identifiers = {
      "$My_name1", "$pc",  "abcd", "_",      "_a",           "_a_", "$", "a_b",
      "kw_this",   "self", "a",    "MyName", "kw_namespace", "föo", "🍫"};
